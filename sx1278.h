...
//#define CS   2
//#define DIO0 3      // Make sure ISR is the good one. PCINT2 is for D0-D7 pin
...
#define SX1278_REG_PPMCORRECTION                      0x27
...
//SX1278_REG_FRF_MSB + REG_FRF_MID + REG_FRF_LSB 434.000Mhz
...
// FRF = 7139328(0x6CF000) for 435.750
//#define SX1278_FRF_MSB                                0x6C        //  7     0     carrier frequency setting: f_RF = (F(XOSC) * FRF)/2^19
//#define SX1278_FRF_MID                                0xF0        //  7     0         where F(XOSC) = 32 MHz
//#define SX1278_FRF_LSB                                0x00        //  7     0               FRF = 3 byte value of FRF registers
...
class SX1278 {
  public:
    SX1278(uint8_t bw, uint8_t sf, uint8_t cr);
    
    uint8_t begin(int8_t cs, int8_t reset, int8_t dio0);
    void    end(void);
    
    uint8_t tx(uint8_t *data, uint8_t length);
  uint8_t txBusy(void);

  uint8_t rxAvailable(uint8_t *data, uint8_t *length);
    uint8_t rxBusy(void); 
    ...
    int16_t getLastPacketRSSI(void);
    void    setPpmError(char err);      // Ferr in Hz / carrier in Mhz
 
  private:
    uint8_t _bw, _sf, _cr, _power, _mode, _sf6length;
    int8_t _cs, _reset, _dio0;
    uint32_t _frequency;
